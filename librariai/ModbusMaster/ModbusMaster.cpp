...
//  txBuffer = (uint16_t*) calloc(ku8MaxBufferSize, sizeof(uint16_t));
  ...
  switch(serialPort)
  {
    case STM32_USART0: _serial = &Serial;  Serial.begin(baud);     break;        
...
//----- Modified Egor Orlenok -----
void ModbusMaster::end(uint8_t serialPort)
{
  switch(serialPort)
  {
    case STM32_USART0:  Serial.end();   break;        
    case STM32_USART1:  Serial1.end();  break;        
    case STM32_USART2:  Serial2.end();  break;
  }
}
//----- End Modified ---------------

uint8_t ModbusMaster::setSlaveID(uint8_t set_id)
{
  _u8MBSlave = set_id;
  return _u8MBSlave;  
}
...
uint16_t ModbusMaster::setResponseTimeout(uint16_t timeout)
{
  ku16MBResponseTimeout = timeout;
  return ku16MBResponseTimeout;
}

uint16_t ModbusMaster::getResponseTimeout(void)
{
  return ku16MBResponseTimeout;
}
...
{
  uint8_t u8ModbusADU[256] = {0};
  uint8_t u8ModbusADUSize = 0;
  uint8_t i, u8Qty;
  uint16_t u16CRC;
  volatile uint32_t u32StartTime;
  ...
  u32StartTime = millis(); // + 2; //5;
  while (u8BytesLeft && !u8MBStatus)
  {
    if (_serial->available() > 0)
    {
#if __MODBUSMASTER_DEBUG__
      digitalWrite(__RED_LED_A__, true);
#endif
...
#if __MODBUSMASTER_DEBUG__
      digitalWrite(__RED_LED_A__, false);
#endif
      delayMicroseconds(T1_5); // Modified Egor Orlenok
    }
    else
    {
#if __MODBUSMASTER_DEBUG__
      digitalWrite(__GREEN_LED_B__, true);
#endif
      if (_idle)
      {
        _idle();
...
#if __MODBUSMASTER_DEBUG__
      digitalWrite(__GREEN_LED_B__, false);
#endif
    }
...
void ModbusMaster::rs485_debug(void)
{
//    digitalWrite(TxEnablePin, HIGH); 
 _preTransmission();

        Serial2.println("\n==================\nTransmit [OK] - This is test words transmission!!!\n==================\n"); 
...
void ModbusMaster::setDebubFlag(uint8_t debFlag)
{
  _debFlag = debFlag;
}
