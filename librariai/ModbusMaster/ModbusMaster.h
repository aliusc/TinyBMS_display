...
#define __RED_LED_A__     (PB13) //  RED
#define __GREEN_LED_B__   (PB12) //  GREEN
...
class ModbusMaster
{
  public:
    ...
    void begin(uint8_t slave, uint8_t serialPort , uint32_t baud);
    void end(uint8_t serialPort);
    void idle(void (*)());
    ...
    */
    ...
    uint8_t setSlaveID(uint8_t);
    uint8_t getSlaveID(void);
    uint16_t setResponseTimeout(uint16_t);
    ...
    bool waitWriteSerial(void);
    void rs485_debug(void);
    void setDebubFlag(uint8_t);
    
  private:
    Stream* _serial;                                             ///< reference to serial port object
    uint32_t _serialPort;
    volatile uint8_t _u8MBSlave;                                 ///< Modbus slave (1..255) initialized in begin()  
    uint16_t T1_5; // inter character time out
    uint16_t T3_5; // frame delay    
    static const uint8_t ku8MaxBufferSize                = 128; //64;   ///< size of response/transmit buffers    
...
//    static const uint16_t ku16MBResponseTimeout          = 2000; ///< Modbus timeout [milliseconds]
    uint16_t ku16MBResponseTimeout;                       // Modified Egor Orlenok
